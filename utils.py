...
def h_param_combination(params):
    h_param_comb = [{}]
...
def train_dev_test_split(data, label, train_data_part, dev_frac_part):

    dev_test_frac = 1 - train_data_part
    x_train, x_dev_test, y_train, y_dev_test = train_test_split(
        data, label, test_size=dev_test_frac, shuffle=True
    )
    x_test, x_dev, y_test, y_dev = train_test_split(
        x_dev_test, y_dev_test, test_size=(dev_frac_part) / dev_test_frac, shuffle=True
    )
